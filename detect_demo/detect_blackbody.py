...
def crosshair(bg, temp):
    bg = Image.fromarray(cv2.cvtColor(bg, cv2.COLOR_BGR2RGB))
    draw = ImageDraw.Draw(bg)
    cross = Image.open('crosshair_v2.png')
    pos1, pos2 = (215, 295), (692, 313)
    pos1t, pos2t = (255, 280), (732, 298)
    bg.paste(cross, pos1, cross)
    bg.paste(cross, pos2, cross)
    font = ImageFont.truetype('arial.ttf', 20)
    draw.text(pos1t, f'{round(temp, 2)}ºC', font=font, fill=(255, 255, 255), stroke_width=2, stroke_fill=(0, 0, 0))
    draw.text(pos2t, f'{round(temp, 2)}ºC', font=font, fill=(255, 255, 255), stroke_width=2, stroke_fill=(0, 0, 0))
    return bg
        ...
        for label, temp, confidence, box in zip(labels, temperatures, confidences, bboxes):
            if threshold[0] <= temp <= threshold[1]:
                ...
                thermal = draw_bbox(thermal, x1 - 3, y1 + 18, x2 - 3, y2 + 18, label,
                                    round(temp, 3), round(confidence.item(), 3))
    combined_img = np.hstack((rgb, thermal))
    combined_img = crosshair(combined_img, center_temp)
    os.makedirs(f"{work_path[:-14]}/debug", exist_ok=True)
    combined_img.save(f"{work_path[:-14]}/debug/{date_time}.jpg")
...
def run(
    ...
    def job(job_path, devicecd, date):
        ...
        for i, det in enumerate(pred):  # per image
            ...
            temp_center = imt.celsius[338, 237]
            if len(det):
