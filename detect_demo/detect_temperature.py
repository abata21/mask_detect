...
from utils.plots import Annotator, colors, save_one_box, colorss
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            tpath = path.replace('\\rgb\\', '\\ir\\')
            imt = flyr.unpack(tpath)
            rgbh, rgbw = im0.shape[:2]
            wr, hr = (480 / rgbw), (640 / rgbh)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    temperature = 330 / (temp + 30) + 31.37144 if distance <= 272.182 else 10.5 / (temp - 265.6) + 36.40
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        annotator.box_label(xyxy, f'{round(distance, 3)} {round(temperature, 3)}', color=colorss[c])
                    if save_crop:
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='weights/fold_v4/fold4.pt', help='model path(s)')
    parser.add_argument('--source', type=str, default=r'C:\Desktop\Py\MaskDetect\mask dataset\yolov2\temperature\rgb', help='0 for webcam')
    # C:\Desktop\Py\MaskDetect\mask dataset\yolov2\test_vid\dong
    # C:\Desktop\Py\MaskDetect\mask dataset\yolov2\distance
    parser.add_argument('--data', type=str, default=ROOT / 'data/coco128.yaml', help='(optional) dataset.yaml path')
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[1440], help='inference size h,w')
