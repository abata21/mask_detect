...
def run(
    ...
    def job(job_path, devicecd, date, file_exist_flag):
...
#                         # Temperature
            ...
            if send_event and events and file_exist_flag == "False":
                ...
                # print("data : ", json.dumps([v for v in events]))
                # print("date : ", f'20{date[:2]}-{date[2:4]}-{date[4:6]} 'f'{date[7:9]}:{date[9:11]}:{date[11:13]}')
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        while True:
            ...
            file_exist_flag = msg[1]
            deviceid = path.split('/')[-2]
            now = path.split('/')[-1]
            combine_img = job(path, deviceid, now, file_exist_flag)
