...
# from app.flask1 import flask_get
...
class yolo_detect():
    def __init__(self,date):
        ...
        # self.data = datetime.datetime.now().strftime("%y%m%d_%H%M%S")
        # self.folder_path = 'C:/Users/abc/Desktop/신종감염병/yolov5/files/feverCamA/230421_164121'
        # self.folder_path = f"C:/Users/abc/Desktop/신종감염병/yolov5/files/hyo/feverCamA/{self.data}"
        self.folder_path = f"C:/Users/abc/Desktop/신종감염병/yolov5/files/hyo/feverCamA/{date}"
        ...
        self.deviceid = self.folder_path.split('/')[-2]
        self.now = self.folder_path.split('/')[-1]

        # self.img_path_q = self.mp.Queue()
    ...
    def cv2_debug_img(self,ir_img, imd, labels, temperatures, confidences, bboxes, now, folder_path, wr, hr, temp_threshold,class_name):
        ...
        os.makedirs(f"{folder_path}/debug", exist_ok=True)
        cv2.imwrite(f"{folder_path}/debug/{now}.jpg", combined_img)
        window_name = 'test'
        cv2.imshow(window_name, combined_img)
        cv2.waitKey(1)
        LOGGER.info(f"Debug image saved to {colorstr('bold', f'{folder_path}/debug/{now}.jpg')}")

    def yolo_model_and_thub_send(self):
        executor = ThreadPoolExecutor(2)
        future2 = executor.submit(flyr.unpack, self.ir_img, )
        ...
        for i, det in enumerate(pred):
            ...
            with open(f'{self.input_path}/output.csv', 'w', newline='') as csv_out:
                ...
                if self.send_event and self.events and self.folder_path == 'False':
                    ...
                    data = MultipartEncoder(fields={'deviceCd': self.deviceid,
                                                    ...
                                                    'date': f'20{self.date[:2]}-{self.date[2:4]}-{self.date[4:6]} '
                                                            f'{self.date[7:9]}:{self.date[9:11]}:{self.date[11:13]}',
    ...
    def multi_processing_flask_and_mask(self):
        ...
        # ex.submit(self.run_flask)

        time.sleep(3)
        print('넘어감')

        # #ir_img를 flyr.py 통해서 온도값 등 나옴
        # executor = ThreadPoolExecutor(2)
        # self.future2 = executor.submit(flyr.unpack, self.ir_img, )
        # executor2 = ThreadPoolExecutor(2)
        # executor2.submit(self.yolo_model_and_thub_send(self.future2))
        # Processes1 = self.mp.Process(target=self.yolo_model_and_thub_send(self.future2), name='flask_get_1') #self.future2
        #
...
def main():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        while True:
            ...
            # print(msg)
            msg = str(msg.decode()).split("/")[-1]
    # date = datetime.datetime.now().strftime("%y%m%d_%H%M%S")
            multi_rtsp = yolo_detect(msg)
            multi_rtsp.yolo_model_and_thub_send()
    # multi_rtsp.multi_processing_flask_and_mask()
# print(str(path_qq.get()))
...
if __name__ == "__main__":
    ...
    # def path_get(self):  ### 이거는 지금 소켓통신에서 q로 할려니까 안됨 왜??
    #     while True:
    #         # if self.img_path_q.qsize() > 0:
    #         self.folder_path = 'C:/Users/abc/Desktop/신종감염병/yolov5/files/feverCamA/230421_162608'
    #         self.input_path = f'{self.folder_path}/input'
    #         self.rgb_img = f'{self.input_path}/photo.jpg'
    #         self.ir_img = f'{self.input_path}/msx.jpg'
    #         self.save_dir = f'{self.input_path}'
...
# def main():
#     yolo = yolo_detect()
#     yolo.yolo_model_and_thub_send()
#
# if __name__ == "__main__":
#     # flask_app.app.run(host="0.0.0.0", port=5000, debug=False)
#     # while True:
#     #     print(str(flask_app.flask_get))
#     main()